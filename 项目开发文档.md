...
心跳检测是长连接服务中非常重要的功能，用于检测客户端是否还保持着连接。如果客户端长时间没有发送消息（处于空闲状态），服务器应该主动关闭连接以释放资源。这个功能的实现参考了 gRPC 的 keepalive 机制。

首先需要在默认配置文件中定义空闲超时的默认值。打开 apps/im/ws/websocket/dufault.go 文件，添加默认的心跳检测超时时间和确认超时时间：
...
const (
	defaultMaxConnectionIdle = time.Duration(5 * time.Minute)  // 默认5分钟空闲超时
	defaultAckTimeout        = time.Duration(30 * time.Second) // 默认30秒确认超时
)
```

这里 defaultMaxConnectionIdle 表示连接的最大空闲时间，如果超过这个时间客户端没有发送任何消息，服务器就会主动关闭连接。defaultAckTimeout 用于后续实现客户端确认机制。

接下来需要在配置选项中添加可配置的空闲超时时间参数。打开 apps/im/ws/websocket/serverOptions.go 文件，在 serverOption 结构体中添加 maxConnectionIdle 字段，并在 newServerOptions 函数中设置默认值，同时添加 WithServerMaxConnectionIdle 方法允许外部配置：
...
type serverOption struct {
	...
	maxConnectionIdle time.Duration  // 新增字段
}
...
func newServerOptions(opts ...ServerOptions) serverOption {
	o := serverOption{
		...
		patten:             "/ws",
		maxConnectionIdle: defaultMaxConnectionIdle,  // 使用默认值
	}
...
这个配置项允许在创建 WebSocket 服务器时自定义空闲超时时间，如果不配置则使用默认值。

然后需要实现具体的心跳检测逻辑。打开 apps/im/ws/websocket/connection.go 文件，在 Conn 结构体中添加空闲时间相关的字段，并在 NewConn 函数中启动心跳检测协程：
...
type Conn struct {
	*websocket.Conn
	...
	idleMu              sync.Mutex   // 保护空闲时间字段
	idle                time.Time   // 记录上次空闲的时间
	maxConnectionIdle   time.Duration // 最大空闲超时时间
...
func NewConn(s *Server, w http.ResponseWriter, r *http.Request) *Conn {
	...
	conn := &Conn{
		...
		maxConnectionIdle:   s.opt.maxConnectionIdle,  // 从服务器配置中获取
	}

	go conn.keepalive()  // 启动心跳检测协程
	return conn
}
```

在创建连接时，会启动一个 keepalive 协程来检测连接状态。同时需要在 ReadMessage 和 WriteMessage 方法中更新空闲时间，因为这两个操作表示客户端是活跃的：

```go
func (c *Conn) ReadMessage() (messageType int, p []byte, err error) {
	// 开始忙碌
	messageType, p, err = c.Conn.ReadMessage()
	c.idle = time.Time{}  // 设置为空时间，表示正在使用
	return
}
...
当读取或写入消息时，会更新 idle 时间。读取消息时设置为 time.Time{} 表示正在使用中，写入完成后设置为当前时间表示开始空闲。

接下来实现核心的 keepalive 检测逻辑：
...
这个方法使用定时器定期检查连接是否空闲。当定时器触发时，首先获取锁读取当前的空闲时间，如果空闲时间为 time.Time{}，说明连接正在使用中，重置定时器继续等待；如果空闲时间不为空，则计算距离上次空闲已经过去的时间，如果超过 maxConnectionIdle 则认为连接已空闲过久，调用服务器的 Close 方法关闭连接；如果还没有超过超时时间，则计算剩余时间并重置定时器。当收到 done 信号时（表示客户端主动关闭连接），退出检测协程。

心跳检测的完整流程如下：客户端连接到 WebSocket 服务器后，服务器启动一个 keepalive 协程；当客户端发送消息时，ReadMessage 和 WriteMessage 方法会更新 idle 时间为当前时间；keepalive 协程定期检查空闲时间，如果超过配置的 maxConnectionIdle（默认5分钟）客户端还没有任何活动，就会主动关闭连接并释放资源。这样可以及时清理无效连接，保证服务器资源的合理利用。

测试
![img](../../../typora/PHOTOS/754110961f8b048b83ce80b4dfaa5d3b_0.png)
![img](../../../typora/PHOTOS/1ebc0cd22fc93610d160fa5d6d398db6_0.png)

###  实现用户与好友私聊功能

#### 构建消息结构体

在 `apps/im/ws/ws/ws.go` 文件中定义聊天消息结构体，用于客户端与服务端之间的消息传递：
...
其中 `MType` 表示消息类型（如文本），`ChatType` 表示聊天类型（单聊或群聊），`ConversationId` 为会话ID，由发送方和接收方的ID组合而成。

#### 实现私聊功能

第一步在 `apps/im/ws/internal/handler/router.go` 文件中注册消息路由，将 `conversation.chat` 方法绑定到聊天处理函数：
...
第二步在 `apps/im/ws/internal/handler/conversation/conversation.go` 文件中实现聊天处理函数，用于接收并处理客户端发起的聊天请求：
...
该函数首先解析客户端发来的消息数据，然后根据聊天类型（单聊或群聊）调用相应的处理逻辑。对于私聊，会调用 `SingleChat` 方法进行消息存储，并通过 `SendByUserId` 方法将消息推送给接收方。

第三步在 `apps/im/ws/internal/logic/conversation.go` 文件中实现私聊的业务逻辑，将消息持久化存储到数据库：
...
该逻辑首先根据发送方和接收方的ID生成会话ID，然后构建聊天日志对象并插入数据库。其中 `wuid.CombineId` 用于合并两个用户ID生成唯一的会话ID，聊天日志包含会话ID、发送方、接收方、消息类型、消息内容等信息。

#### 业务流程分析

私聊的完整流程如下：客户端通过 WebSocket 连接到服务器后，携带 token 进行身份验证；验证通过后，客户端发送 `conversation.chat` 方法的消息，服务器接收并解析消息内容；根据聊天类型调用对应的处理逻辑，将消息持久化到数据库，并通过 `SendByUserId` 方法根据接收方的用户ID将消息推送给对应的连接。整个过程实现了消息的发送、存储和实时推送功能。



测试

![image-20260301132149101](D:\typora\PHOTOS\image-20260301132149101.png)![image-20260301132153930](D:\typora\PHOTOS\image-20260301132153930.png)

### 引入kafka

![image-20260301181312547](D:\typora\PHOTOS\image-20260301181312547.png)

#### 引入kafka消息中间件,并构建异步队列服务

在分布式IM系统中引入 Kafka 消息队列可以实现消息的异步处理和削峰填谷，提高系统的吞吐量和可靠性。第一步需要在项目中安装 go-zero 框架提供的 kq 依赖包：

```bash
go get github.com/zeromicro/go-queue/kq
```

第二步在 `apps/task/mq/etc/dev/task.yaml` 文件中配置 Kafka 消费者信息：
...
MsgChatTransfer:
  Name: msg.chat.transfer
  Brokers:
    - 192.168.88.130:9092
  Group: kafka
  Topic: msg.chat.transfer
  Offset: first
  Consumers: 1
```

其中 `Name` 为服务名称，`ListenOn` 为服务监听地址，`MsgChatTransfer` 配置了 Kafka 消费者参数：`Brokers` 为 Kafka broker 地址，`Group` 为消费者组名称，`Topic` 为要订阅的主题，`Offset` 为消费位置（first 表示从最早消息开始消费），`Consumers` 为消费者数量。

第三步在 `apps/task/mq/internal/config/config.go` 文件中定义配置结构体，用于加载 Kafka 消费者配置：
...
其中 `kq.KqConf` 是 go-zero 框架提供的 Kafka 消费者配置结构体，包含了连接 Kafka 所需的所有参数。

第四步在 `apps/task/mq/internal/svc/servicecontext.go` 文件中创建服务上下文，将配置传递给业务处理层：
...
第五步在 `apps/task/mq/internal/handler/msgTransfer/msgTransfer.go` 文件中实现 Kafka 消费者处理逻辑，用于消费消息并进行处理：
...
该结构体实现了 `Consumer` 接口的 `Consume` 方法，当 Kafka 主题中有消息时，会自动调用该方法进行处理。`key` 为消息的键，`value` 为消息的内容。目前该实现仅为打印消息，实际业务中可以在此方法中实现消息的持久化存储、转发等逻辑。

第六步在 `apps/task/mq/internal/handler/listen.go` 文件中创建消息监听器，用于启动 Kafka 消费者：
...
该文件创建了消息监听器，通过 `kq.MustNewQueue` 方法创建 Kafka 消费者队列，并将消息处理逻辑 `MsgChatTransfer` 注入。`Services()` 方法返回所有需要启动的服务，支持同时启动多个消费者处理不同的消息主题。

第七步在 `apps/task/mq/task.go` 文件中创建主入口程序，用于启动消息队列服务：
...
该主程序首先加载配置文件，然后创建一个服务组 `serviceGroup`，用于管理多个服务实例。接着创建服务上下文和消息监听器，将监听器中的所有服务添加到服务组中，最后启动服务组。服务组会自动管理协程的生命周期，实现优雅退出。

#### 基于kafka异步数据存储落地及消息通信

![alt text](65ebe9810943f73a08580290.png)

##### websocket客户端

在 `apps/im/ws/internal/handler/conversation/conversation.go` 的 Chat 方法中，当 WebSocket 服务接收到用户发送的消息后，会将消息推送到 Kafka 消息队列中进行异步处理：
...
该代码的作用是接收用户发送的聊天消息，根据聊天类型生成会话ID，然后通过 MsgChatTransferClient 将消息推送到 Kafka 队列，实现消息的异步处理和存储。

##### mq中的服务业务

在 `apps/task/mq/internal/handler/msgTransfer/msgTransfer.go` 中，MQ 服务定义了消息消费者 Consume 方法，用于处理从 Kafka 消费到的消息：
...
该代码的作用是从 Kafka 消费到聊天消息后，首先调用 addChatLog 方法将消息持久化存储到 MongoDB 数据库，然后通过 WsClient 将消息推送给接收方，实现消息的存储和实时推送。

##### websocket将接收消息写入消息队列

在 `apps/task/mq/mqclient/msgtransfer.go` 中，定义了 MsgChatTransferClient 客户端用于向 Kafka 推送消息：
...
该代码的作用是封装 Kafka 生产者客户端，提供 Push 方法将消息序列化后推送到指定的 Kafka 主题，WebSocket 服务通过依赖注入的方式使用该客户端将消息写入消息队列。

### 消息ack机制

![image-20260302112500538](D:\typora\PHOTOS\image-20260302112500538.png)

<img src="D:\typora\PHOTOS\image-20260302183126686.png" alt="image-20260302183126686" style="zoom: 80%;" />

#### 连接属性设置

在 `apps/im/ws/websocket/serverOptions.go` 中定义了服务端的可选配置项结构体，添加了 ACK 机制相关的配置字段：
...
该代码的作用是定义服务端配置选项，添加了 `ack` 字段用于指定 ACK 确认模式，以及 `ackTimeout` 字段用于设置超时时间。在初始化时默认使用 `NoAck` 模式，即不启用消息确认机制。

同时在 `apps/im/ws/websocket/serverOptions.go` 中添加了 ACK 机制的配置函数：
...
该代码的作用是提供链式配置方法，允许在创建 WebSocket 服务时通过 `WithServerAck` 设置 ACK 模式，通过 `WithServerAckTimeout` 设置超时时间。

在 `apps/im/ws/im.go` 中启用 ACK 机制：
...
该代码的作用是在 WebSocket 服务启动时启用 RigorAck（严格确认）模式，确保消息可靠传递。

#### 实现收消息ack机制

在 `apps/im/ws/websocket/connection.go` 中，连接结构体定义了消息确认队列相关字段：
...
该代码的作用是定义消息待确认队列 `readMessage` 和消息序列映射 `readMessageSeq`，用于存储已收到但尚未收到客户端确认的消息，实现消息可靠传输的确认机制。

在 `apps/im/ws/websocket/connection.go` 中实现了消息入队方法 `appendMsgMq`：
...
该代码的作用是当服务端收到消息时，将消息加入待确认队列，并检查是否是重复消息。如果消息已存在于队列中且确认序号不小于当前序号，则判定为重复消息直接丢弃；如果收到的是 ACK 确认帧但没有对应的消息记录，也直接忽略。从而避免客户端重复发送导致的消息重复处理问题。

#### ACK模式判断与处理

在 `apps/im/ws/websocket/server.go` 中实现了消息确认类型的定义和处理方法：
...
该代码的作用是定义 ACK 确认模式的枚举类型，包括 `NoAck`（不启用确认）、`OnlyAck`（简单确认）和 `RigorAck`（严格确认）三种模式，用于控制消息可靠传输的级别。

在 `apps/im/ws/websocket/server.go` 中实现了判断是否需要 ACK 的方法：
...
该代码的作用是判断当前消息是否需要 ACK 确认。如果传入的 message 为 nil，则判断服务端是否启用了 ACK 机制；如果 message 不为 nil，则判断消息类型是否为非 ACK 类型且服务端已启用 ACK 机制。

#### 服务端ACK消息处理流程

在 `apps/im/ws/websocket/server.go` 的 `readAck` 方法中实现了服务端处理 ACK 的核心逻辑：
...
该代码的作用是实现服务端对消息确认的处理流程。当启用 `OnlyAck` 模式时，服务端直接回复 ACK 确认并从队列中移除消息；当启用 `RigorAck` 模式时，服务端先发送 ACK 确认，等待客户端返回处理结果后再进行二次确认，如果客户端超时未响应则重发 ACK，如果超时时间过长则丢弃消息。从而实现可靠的消息传输机制，确保每条消息都被正确处理。

 测试ack

![image-20260302185636997](D:\typora\PHOTOS\image-20260302185636997.png)

![image-20260302185640834](D:\typora\PHOTOS\image-20260302185640834.png)

### 消息拉取

消息拉取功能允许用户根据会话ID查询聊天记录。首先需要在im.proto中定义获取聊天记录的rpc方法。
...
在im/rpc的logic中实现具体的查询逻辑。

> apps/im/rpc/internal/logic/getchatloglogic.go
...
该方法支持两种查询方式：根据msgId查询单条消息记录，或者根据会话ID和时间范围查询消息列表。查询结果转换为ChatLog结构返回。

### 消息标记已读

消息已读功能采用WebSocket实时处理+MQ异步消费的架构。当用户阅读消息时，通过WebSocket发送已读请求，服务器将请求推送到消息队列，由专门的消费者处理已读状态的更新和通知推送。

首先定义WebSocket处理路由。

> apps/im/ws/internal/handler/router.go
...
在WebSocket处理器中接收已读请求并推送到消息队列。

> apps/im/ws/internal/handler/conversation/conversation.go
...
一个一个start太麻烦，可以利用go-zero中的service.Service接口，将消息队列的消费者注册到go-zero的服务中。(因为只要实现了start和stop方法，就相当于实现了service.Service接口)

> apps/task/mq/internal/handler/listen.go
...
实现已读消息的消费者逻辑，包括更新数据库中的已读状态和推送通知。

> apps/task/mq/internal/handler/msgTransfer/msgReadTransfer.go
...
该实现使用位图（bitmap）来存储群聊中的已读用户信息，这样可以高效地记录多个用户的已读状态。对于单聊，只需要简单标记为已读即可。

对于已读消息的延迟聚合处理，可以配置GroupMsgReadHandler为DelayTransfer模式，延迟处理已读消息，减少频繁的数据库更新和推送。
...
在WebSocket服务中接收并处理已读推送通知，根据聊天类型分别推送给对应用户。
...
> apps/im/api/internal/logic/getchatlogreadrecordslogic.go
...
该实现首先获取消息记录，然后根据聊天类型分别处理。单聊直接判断已读状态，群聊则遍历群成员并使用位图判断每个成员是否已读，最后返回已读和未读用户的手机号列表。
...
在群聊场景中，如果短时间内有多位用户同时阅读同一条消息，会导致以下问题：每次用户阅读消息时，系统都需要更新数据库中的已读记录，并将已读通知推送给所有群成员。当大量用户在短时间内阅读消息时，会造成频繁的数据库更新和大量的推送通知，给系统带来性能压力。

针对这个问题，可以采用延迟聚合的优化方案。其核心思想是将短时间内收到的多个已读请求合并成一个批次进行处理，从而减少数据库操作次数和推送次数。
...
配置项说明：`GroupMsgReadHandler` 表示已读消息的处理模式，0 表示即时处理（不合并），1 表示延迟聚合处理；`GroupMsgReadRecordDelayTime` 表示延迟时间（秒），超过这个时间后推送合并后的消息；`GroupMsgReadRecordDelayCount` 表示最大合并数量，当合并的已读记录数超过这个数量时立即推送。

在本地配置文件中开启延迟聚合模式。

> apps/task/mq/etc/local/mq.yaml
...
修改 MsgReadTransfer 结构体，添加延迟处理所需的字段。
...
在创建 MsgReadTransfer 时，根据配置初始化延迟参数，并启动消息聚合协程。
...
在 Consume 方法中，根据配置决定是立即推送还是延迟聚合。
...
case constants.GroupChatType:
    ...
    if _, ok := m.groupMsgs[push.ConversationId]; ok {
        ...
        // 合并请求
        m.groupMsgRead[push.ConversationId].mergePush(push)
    } else {
...
定义 groupMsgRead 结构体，用于管理群消息的聚合逻辑。
...
实现 mergePush 方法，将新的已读记录合并到现有的已读记录中。
...
实现 transfer 方法，处理超时推送和超量推送两种触发条件。
...
该优化方案通过延迟聚合的方式，有效减少了数据库操作次数和推送次数。当配置 `GroupMsgReadHandler: 1` 后，系统会将同一会话的已读请求在指定时间或数量内进行合并，然后一次性推送给群成员。这样可以显著降低系统压力，提升整体性能。

### 好友列表和群成员在线离线情况

在即时通讯应用中，展示好友和群成员的在线状态是一个常见需求。通过在用户登录时记录在线状态到 Redis，可以高效地查询好友和群成员的在线情况。

首先定义 Redis 中存储在线用户信息的 key。
...
该代码在用户登录成功后，使用 Redis 的 Hash 结构存储用户的在线状态，其中 key 为 "online:user"，field 为用户 ID，value 为 "1" 表示在线。

在 social.api 中定义查询好友在线状态的接口。
...
在 social/api 的路由中注册该接口。

> apps/social/api/internal/handler/routes.go

实现查询好友在线状态的业务逻辑。

> apps/social/api/internal/logic/friend/friendsonlinelogic.go
...
该实现首先获取当前用户的好友列表，然后从 Redis 中获取所有在线用户的哈希表，遍历好友列表匹配在线状态，返回好友ID到在线状态的映射。

在 social.api 中定义查询群成员在线状态的接口。
...
> apps/social/api/internal/logic/group/groupuseronlinelogic.go
...
该实现首先获取指定群聊的群成员列表，然后从 Redis 中获取所有在线用户的哈希表，遍历群成员列表匹配在线状态，返回群成员ID到在线状态的映射。

该方案利用 Redis 的 Hash 数据结构存储用户在线状态，具有以下优点：查询效率高，HGETALL 一次操作即可获取所有在线用户；存储空间小，只需要存储在线用户的 ID；实现简单，登录时设置在线状态，查询时直接读取 Redis 即可。
![image-20260305220337127](D:\typora\PHOTOS\image-20260305220337127.png)

### 幂等性实现

在社交场景中，用户可能会因为网络延迟、重复点击等原因发送重复请求，比如重复创建群聊、重复申请入群等。为了避免这些重复操作导致的数据不一致问题，我们需要实现幂等性机制。

#### 1. RPC 幂等性拦截器实现

首先创建 RPC 幂等性拦截器，该拦截器分为客户端和服务端两部分。
...
在 RPC 服务的启动文件中注册幂等性拦截器。

> apps/social/rpc/social.go
...
创建 HTTP 层的幂等性中间件，用于处理 HTTP 请求的幂等性。
...
@server (
	...
	group:  group
	jwt:    JwtAuth
	middleware: IdempotenceMiddleware
)
service social {
	@doc "创群"
	@handler createGroup
...
```bash
cd apps/social/api
goctl api go -api social.api -dir ./
```

goctl 会自动在 `internal/handler/routes.go` 中注册中间件：
...
1. **客户端生成唯一请求 ID**：每次请求时，客户端拦截器会生成一个唯一的 UUID，并将其放入 gRPC 的 Metadata 中传递到服务端。

2. **服务端验证请求**：服务端拦截器从 Metadata 中获取请求 ID，检查该方法是否需要幂等性处理。

3. **分布式锁控制**：使用 Redis 的 SETNX 命令实现分布式锁，确保同一请求 ID 只有一个请求能执行。

4. **结果缓存**：请求执行完成后，将结果缓存到内存 Cache 中，如果后续有相同请求 ID 的请求，直接返回缓存的结果。

5. **重试策略**：RPC 客户端配置了重试策略，当遇到 UNKNOWN 或 DEADLINE_EXCEEDED 错误时，会自动重试最多 5 次。

目前只对 `/social.social/GroupCreate`（创建群聊）方法开启了幂等性支持，如果需要支持更多方法，可以在 `pkg/interceptor/idempotence.go` 的 `method` map 中添加：

```go
method: map[string]bool{
    "/social.social/GroupCreate": true,
    "/social.social/GroupPutIn": true,    // 申请入群
    "/social.social/FriendPutIn": true,    // 好友申请
}
```

#### 7. 代码作用和意图

这段幂等性代码在整个项目中充当**请求防护层**的角色，主要作用包括：

1. **防止重复提交**：当用户快速点击多次提交按钮时，只有第一次请求会被执行，后续的重复请求会直接返回第一次的结果。

2. **处理网络超时重试**：当网络超时导致客户端重试时，服务端能够识别出这是同一个请求，避免重复执行。

3. **保证数据一致性**：在分布式系统中，确保同一个操作无论执行多少次，产生的结果都是相同的，不会产生副作用。

4. **提升用户体验**：避免因重复提交导致的错误提示，让用户操作更加流畅。

5. **降低系统负载**：通过缓存执行结果，减少重复请求对数据库和业务逻辑的冲击。

实现的核心思想是**请求唯一标识 + 分布式锁 + 结果缓存**，这是一种在分布式系统中实现幂等性的经典方案。
